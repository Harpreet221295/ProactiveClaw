...
def _run_cdp(method: str, params: dict | None = None, timeout: float = 30) -> dict:
    ...
    print(f"[browser-debug] _run_cdp: method={method}, loop={'set' if loop else 'None'}")
    if loop is None:
        raise RuntimeError("Event loop not set — is the server running?")

    import slack_server
    print(f"[browser-debug] _run_cdp: _extension_ws={slack_server._extension_ws is not None}")

    coro = slack_server.send_cdp_command(method, params, timeout=timeout)
