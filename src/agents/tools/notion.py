...
def _try_fix_json(s: str) -> str:
    """Attempt to fix common LLM JSON errors (missing brackets/braces)."""
...
def create_database_entry(database_id: str, properties_json: str) -> str:
    try:
        ...
        # Attempt to fix minor JSON issues from LLM output
        try:
            properties = json.loads(_try_fix_json(properties_json))
            print(f"  [notion] Auto-fixed malformed JSON in create_database_entry")
        except json.JSONDecodeError:
            return f"Error: malformed JSON in properties_json — could not parse."

    # Auto-fix: Status property must use "status" type, not "select"
    if "Status" in properties:
        status_val = properties["Status"]
        if "select" in status_val and "status" not in status_val:
            properties["Status"] = {"status": status_val["select"]}
            print(f"  [notion] Auto-fixed Status: select → status")

    # Auto-fix: if a property is sent as {"url": "..."} but the DB expects rich_text,
    # convert it. We do this by catching the error and retrying (see below).
    # Also fix common type mismatches proactively for known properties.
    for prop_name, prop_val in list(properties.items()):
        # Convert {"url": "..."} to rich_text if it looks like a URL-only value
        if "url" in prop_val and len(prop_val) == 1 and prop_name not in ("URL",):
            url_str = prop_val["url"]
            properties[prop_name] = {"rich_text": [{"text": {"content": url_str}}]}
            print(f"  [notion] Auto-fixed {prop_name}: url → rich_text")
    ...
    try:
        ...
        error_str = str(e)
        if "is not a property that exists" in error_str or "is expected to be" in error_str:
            import re
            # Match property names that may contain slashes, spaces, special chars
            bad_props = re.findall(r"([A-Za-z0-9_/\- ]+?) is not a property that exists", error_str)
            type_mismatches = re.findall(r"([A-Za-z0-9_/\- ]+?) is expected to be", error_str)
            for prop in bad_props + type_mismatches:
                prop = prop.strip()
                if prop in properties:
                    properties.pop(prop)
                    print(f"  [notion] Removed bad property: {prop}")
            if not properties.get("title") and not any(
                "title" in str(v) for v in properties.values()
            ):
                return f"Error: all properties were invalid. Notion API said: {error_str}"
            body["properties"] = properties
            try:
                page = _notion_request("POST", "/pages", body)
            except RuntimeError as e2:
                return f"Error creating entry (retry failed): {e2}"
        else:
            raise
